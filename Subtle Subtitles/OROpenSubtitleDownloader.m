...
- (void)downloadSubtitlesForResult:(OpenSubtitleSearchResult *)result toPath:(NSString *)path :(void(^)(NSString *path, NSError *error))onResultsFound
{
    ...
    NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request
                                                                  ...
                                                                  destination:^NSURL *(NSURL *targetPath, NSURLResponse *response)
                                              {
                                                  ...
                                                  return requireUnzipping? [NSURL fileURLWithPath:suggestedPath] : (NSURL *)path;
                                              }
...
+ (OpenSubtitleSearchResult *)resultFromDictionary:(NSDictionary *)dictionary {
    ...
    object.subtitleRating = [dictionary[@"SubRating"] floatValue];
    ...
    object.subtitlePage = dictionary[@"SubtitlesLink"];
    object.hearingImpaired = dictionary[@"SubHearingImpaired"];
    object.downloadCount = [dictionary[@"SubDownloadsCnt"] integerValue];
    object.bad = dictionary[@"SubBad"];
    object.movieName = dictionary[@"MovieName"];    // Modifié par ici
    object.subtitleLength = dictionary[@"SubLastTS"];
    object.subtitleSize = [dictionary[@"SubSize"] integerValue];
    object.hd = dictionary[@"SubHD"];
    object.subAddDate = dictionary[@"SubAddDate"];
